...
close all
figure(1)
EX = mu + (X0-mu)*exp(-alpha*t); % expected path It is the same of the Ornstein-Uhlenbeck process
...
var1 = (sigma^2/alpha)*(exp(-alpha*t)-exp(-2*alpha*t));
var2 = mu*(sigma^2/(2*alpha))*(1-exp(-alpha*t)).^2;
...
figure(2)
plot(t,var3,'b', t,Xvar,'g', t,mean((X-transpose(EX)).^2,2),'r')
legend('Theory', 'Sample1', 'Sample2')
xlabel('t')
ylabel('Var(X)')
