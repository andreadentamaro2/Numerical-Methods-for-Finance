...
d2 = (log(St/K)+muRN*(T-t))./(sigma*sqrt(T-t));
d1 = d2 + sigma*sqrt(T-t);

close all
figure(1)
Vc = St.*exp(-q*(T-t)).*cdf('norm',d1,0,1) - K*exp(-r*(T-t)).*cdf('norm',d2,0,1);
...
figure(2)
Vp = K*exp(-r*(T-t)).*cdf('norm',-d2,0,1) - St.*exp(-q*(T-t)).*cdf('norm',-d1,0,1);
...
d2 = (xt-k+muRN*(T-t))./(sigma*sqrt(T-t));
d1 = d2 + sigma*sqrt(T-t);

figure(3)
Vc = S0*(exp(xt-q*(T-t)).*cdf('norm',d1,0,1) - exp(k-r*(T-t)).*cdf('norm',d2,0,1));
...
figure(4)
Vp = S0*(exp(k-r*(T-t)).*cdf('norm',-d2,0,1) - exp(xt-q*(T-t)).*cdf('norm',-d1,0,1));
