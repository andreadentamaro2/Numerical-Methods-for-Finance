...
t = 0:dt:T; % observation times / time grid
%mu = 0.12; sigma = 0.4; % two choices of model parameters
...
close all
figure(1)
EX = mu*t; % expected path (Theoretical quantity)
plot(t,EX,'k',t,mean(X),':k',t,X(1:1000:end,:),t,EX,'k',t,mean(X),':k')
% grid t and expected path in black // experimental expected path (mean of
% our matrix and it is in black dots // Plot one path every 1000 so we have
% 20 paths // Write again first 2 paths because you don't want them hidden
% in the diagram (they were wrote at the beggining for the legend)
...
figure(2)
plot(t,sigma^2*t,t,var(X)) % sigma^2*t is the theoretical variance // var(x) is the variance computed by the Montecarlo simulation
legend('Theory: \sigma^2t = 2Dt','Sampled','Location','NorthWest') % Location NorthWest moves the legend to the left
xlabel('t')
